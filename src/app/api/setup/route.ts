  ...
  try {
    ...
    if (existing.length === 0) {
      for (let i = 0; i < seedProducts.length; i++) {
        ...
        await sql`INSERT INTO products (name, name_mr, emoji, category, unit, price_per_unit, sort_order)
          VALUES (${p.name}, ${p.mr}, ${p.emoji}, ${p.category}, ${p.unit}, ${p.price.toFixed(2)}, ${i})`;
      }
    }
    ...
    return NextResponse.json({
      ...
      credentials: {
        admin: { email: "admin@wholesale.local", password: "admin123" },
